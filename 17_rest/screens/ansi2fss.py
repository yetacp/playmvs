...
def main():
    ...
    with open(sys.argv[1], "r", encoding = 'latin-1') as f:
        with open(sys.argv[1]+".c", 'w') as c_file:
            ...
            c_file.write("""/* DO NOT EDIT {filename}.c - generated on: {date} */
#include "fss.h"
#include "{filename}.h"
...
""".format(date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    if ord(byte) == ord('\x1b'):
                        if word_end:
                            ...
                            if variable:
                                ...
                                variable_name = "f{count:05d}".format(count = symbol_count)
                                symbols[variable_name] = string
                            writeline(c_file,variable_name, string, current_color, background_color, s_x, s_y)
                            background_color = '0'
                            string = ''
                        escaped = True
                        continue
        ...
        with open(sys.argv[1]+".h", 'w') as h_file:
